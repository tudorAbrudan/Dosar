...
import { createProperty } from '@/services/entities';
...
const DOC_OPTIONS: DocOption[] = [
  { type: 'act_proprietate', label: 'Act proprietate' },
  { type: 'cadastru', label: 'Cadastru' },
  { type: 'factura', label: 'Factură / Impozit' },
];

export default function WizardProprietateScreen() {
  ...
  async function handleSave() {
    if (!name.trim()) {
      Alert.alert('Eroare', 'Introdu numele proprietății.');
      return;
    }
    setLoading(true);
    try {
      const property = await createProperty(name.trim());
      for (const type of selected) {
        const expiry_date = expiries[type]?.trim() || undefined;
        await createDocument({ type, property_id: property.id, expiry_date });
      }
      await scheduleExpirationReminders();
      router.push(`/(tabs)/entitati/${property.id}`);
    } catch (e) {
      ...
      >
        ...
        >
          ...
          <View style={styles.stepBlock}>
            ...
            <ThemedTextInput
              style={styles.input}
              placeholder="ex. Apartament Str. Florilor"
              value={name}
