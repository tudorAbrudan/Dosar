...
// Sync version for tests using a simple JS SHA-256 fallback
export function hashManifest(canonical: string): string {
  return sha256Hex(canonical);
}

// Pure JS SHA-256 (compact implementation, ~50 lines)
function sha256Hex(message: string): string {
  const utf8 = new TextEncoder().encode(message);
  const buf = new Uint8Array(((utf8.length + 9 + 63) >> 6) << 6);
  buf.set(utf8);
  buf[utf8.length] = 0x80;
  const lenBits = utf8.length * 8;
  const view = new DataView(buf.buffer);
  view.setUint32(buf.length - 4, lenBits >>> 0, false);
  view.setUint32(buf.length - 8, Math.floor(lenBits / 0x100000000), false);
  const K = [
