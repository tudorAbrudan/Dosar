...
import { encryptField, decryptField, decryptFieldOrNull } from './medicalCrypto';
...
export async function retrieveContext(recordId: string, query: string): Promise<RetrievedContext> {
  ...
  const seenObsIds = new Set(observations.map(o => o.id));
  const docChunks: RetrievedDocChunk[] = [];
  let charCount = 0;
  for (const h of ftsHits) {
    if (h.source_type === 'observation') {
      if (seenObsIds.has(h.source_id)) continue;
      // Observații suplimentare aduse de FTS dar nu de structured lookup —
      // încărcăm și le adăugăm la set.
      const extra = allObs.find(o => o.id === h.source_id);
      if (extra && observations.length < 40) {
        observations.push(extra);
        seenObsIds.add(extra.id);
      }
      continue;
    }
    if (charCount + h.chunk_text.length > CONTEXT_CHAR_CAP) break;
    const doc = await getDocumentById(h.source_id);
    if (!doc) continue;
    docChunks.push({
      documentId: doc.id,
...
const SYSTEM_PROMPT = `Ești asistent pentru dosarul medical al utilizatorului în aplicația Dosar.
